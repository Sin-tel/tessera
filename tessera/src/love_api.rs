...
impl LuaUserData for Graphics {
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_function("newImage", |lua, filename: String| {
            ...
            let image_id = state
                .canvas
                .load_image_file(&filename, ImageFlags::empty())
                .unwrap();
        ...
        methods.add_function(
            "intersectScissor",
            |lua, (x, y, w, h): (f32, f32, f32, f32)| {
                let mut state = lua.app_data_mut::<State>().unwrap();

                let (new_x, new_y, new_w, new_h) =
                    if let Some((sx, sy, sw, sh)) = state.current_scissor {
        ...
        methods.add_function("print", |lua, (text, x, y): (String, f32, f32)| {
            ...
            state
                .text_engine
                .draw_text(&mut state.canvas, &text, x, y, &paint);
        ...
        methods.add_function(
            "ellipse",
            |lua, (mode, x, y, w, h): (String, f32, f32, f32, f32)| {
                let mut state = lua.app_data_mut::<State>().unwrap();

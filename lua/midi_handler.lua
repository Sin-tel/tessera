...
local function handle_midi(event)
	...
	if index ~= -1 then
		if event.name == "note on" then
			...
			backend.send_note_on(index, tracks[new_i].note + tracks[new_i].offset, event.vel)
		end
		...
		if event.name == "note off" then
			for _, b in ipairs(tracks) do
				if b.note == event.note then
					...
					b.age = 0
					b.vel = 0
					backend.send_cv(index, b.note + b.offset, 0)
					-- b.note = -1
		...
		if event.name == "CC" then
			for _, b in ipairs(tracks) do
				if b.channel == event.channel then
					if event.cc == 74 then
						b.vel = event.y
						backend.send_cv(index, b.note + b.offset, b.vel)
						break
					end
				end
			end

...
function channelView:update()
	...
	if self.box.focus then
		self.index = math.floor((my + self.scroll) / ui.GRID) + 1
	end
end
...
function channelView:draw()
	...
	for i, v in ipairs(channelHandler.list) do
		local y = ui.GRID * (i - 1) - self.scroll

		if self.index == i then
			love.graphics.setColor(theme.bg_highlight)
			love.graphics.rectangle("fill", 0, y, w, ui.GRID)
		end
		...
		util.drawText(v.name, 0, y, w - ui.BUTTON_SMALL * 5, ui.GRID, "left")
